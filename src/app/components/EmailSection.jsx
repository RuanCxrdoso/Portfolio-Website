...
export function EmailSection() {
  ...
  const txt = useTranslations('EmailSection')
  ...
  return (
    <section id="contact" className="grid md:grid-cols-2 my-12 md:my-12 py-24 gap-4 relative">
      ...
      >
        ...
        <p className="text-[#ADB7BE] mb-4 w-4/5 sm:max-w-md">
          {txt('description')}
        </p>
      ...
      >
        <h5 className="text-xl font-bold text-white py-2 mt-6 sm:mt-0 sm:-mb-4">
          {txt('rightTitle')}
        </h5>
        <div className="flex flex-col gap-12 sm:gap-10 justify-center items-start -ml-4 sm:ml-0 sm:w-full">
          ...
          >
            <EnvelopeIcon width={35} height={35} className="block sm:hidden xl:block"/>
            <span className="text-md sm:text-sm lg:text-xl font-semibold">cardosoruan2001@gmail.com</span>
            <ArrowRightIcon width={20} height={20} className="absolute right-8 hidden xl:block transform group-hover:translate-x-2 transition-transform duration-300"/>
          </Link>
          <Link
            href="https://wa.me/55071986944657"
            className="group flex justify-start items-center gap-4 bg-[#141414] p-4 px-6 sm:px-8 rounded-xl w-full sm:w-4/5 relative transform hover:-translate-y-2 transition-transform duration-300"
          >
            <Image src={WhatsAppLogo} alt="" height={35} width={35} className="block sm:hidden xl:block"/>
            <span className="text-md sm:text-sm lg:text-xl font-semibold">{txt('linkText')}</span>
            <ArrowRightIcon width={20} height={20} className="absolute right-8 hidden xl:block transform group-hover:translate-x-2 transition-transform duration-300"/>
          </Link>
        </div>
      </motion.div>
